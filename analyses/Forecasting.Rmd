...
```
The model for clicks appears credible, producing click counts in the ballpark of the previous days and weeks. Let's try fitting a similar model for conversions:
```{r}
...
```{r}
library(tidyr)
last_100_days <- spread(data = plotter, key = type, value = success_rate)[1:100,] # generate the wide data format from my plotter data frame for convenient vector calculations
...
Let's find the best moving average given the data:
```{r}
library(smooth)
